...
<div class="md:w-1/2 mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
  ...
  <form action="{{ route('tasks.create') }}" method="POST" class="w-full max-w-xl mx-auto px-4 py-2">
    @csrf
    <div class="flex items-center border-b-2 border-teal-500 py-2">
      <input class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" name="task_name" placeholder="タスクを追加してください" />
  ...
  @if ($tasks->isEmpty())
    ...
    <li class="flex items-center py-4 justify-between">
      ...
      <div class="flex space-x-2">
        ...
        <button class="flex-shrink-0 bg-sky-500 hover:bg-sky-700 border-sky-500 hover:border-sky-700 text-sm border-4 text-white py-1 px-2 rounded">編集</button>
        <form action="{{ route('tasks.moveToTrash', $task->id) }}" method="POST">
          @csrf
